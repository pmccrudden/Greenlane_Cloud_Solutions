...
# This ensures tenant login works correctly

echo "=== Deploying Multi-Tenant Login Fixed Version to Cloud Run ==="
...
# Create a timestamp tag for the image
TIMESTAMP=$(date +%Y%m%d%H%M%S)
IMAGE_TAG="tenant-login-fix-${TIMESTAMP}"
IMAGE_NAME="gcr.io/greenlane-cloud-solutions/greenlane-crm-${IMAGE_TAG}"

# Build the Docker image
echo "Building Docker image ${IMAGE_NAME}..."
docker build -t "${IMAGE_NAME}" -f Dockerfile.simple-fix .
...
# Add ENABLE_FIXED_TENANT_LOGIN=true to indicate we're using the fixed version
ENV_VARS="$ENV_VARS,ENABLE_FIXED_TENANT_LOGIN=true"
...
if [ -n "$SERVICE_URL" ]; then
  ...
  echo ""
  echo "View logs:"
  echo "https://console.cloud.google.com/logs/query;query=resource.type%3D%22cloud_run_revision%22%20resource.labels.service_name%3D%22${SERVICE_NAME}%22?project=greenlane-cloud-solutions"
else
